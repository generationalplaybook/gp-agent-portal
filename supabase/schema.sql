...
-- ─────────────────────────────────────────────────────────────
-- 41. Annuity field set (added 9/4) — Karina: an annuity's product form was showing the same
-- fields as a term life policy (the term checkbox, Face amount/Premium, life-insurance riders),
-- none of which fit. Adds a purpose-built set of fields, shown only when product_type =
-- 'Annuity' (see ProductsSection.tsx/ProductRow.tsx):
...
-- `contract_value` — the current accumulation value, a manually-updated snapshot (annuities
-- don't have a "face amount" the way life insurance does).
...
-- `clients.turned_59_half_notice_sent` — same one-time-only pattern as turned_18_notice_sent
-- (section with the family-linking feature), for a new cron milestone: a client who holds an
-- Annuity product gets an advisor reminder the day they turn 59 1/2, since the IRS's 10% early-
-- withdrawal penalty no longer applies to their annuity from that point on. See
-- src/lib/family.ts (isHalfBirthdayToday) and the check-birthdays cron route.
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- 43. Advisor "Remove access" + client reassignment (added 9/6) — Karina wants an admin to be
-- able to remove an advisor's portal access without going into Supabase directly, and to
-- reassign that advisor's book of business afterward (to one person, or split across several).
...
-- "Unassigned Clients" admin-only section (see AdminInvitePage) rather than deleting anything or
-- silently leaving it under a login nobody can use anymore. An admin then reassigns each client
-- (one at a time, or in a batch) to whichever advisor is taking over that piece of the book — see
-- reassignClients in admin/invite/actions.ts, which also moves that client's reminders and
-- meetings (both carry their own separate agent_id, not just client_id) to the new owner, so
-- nothing pending silently disappears from view for either advisor.
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- 46. Auto-follow-up reminder tracked back to its product (added 9/8, later same day) — Karina,
-- after seeing several leftover "Try again — couldn't reach..." reminders on the Reminders page
-- for rows she'd already hit Undo on: "if you do undo, the reminder should get removed as well...
-- it's autogenerated." Before this, markOutreachOutcome's auto-created reminder had no link back
-- to the product row that created it, so undoOutreachOutcome had no way to find and delete it —
-- outreach_reminder_id closes that gap: set to the new reminder's id the moment
-- markOutreachOutcome creates one (unreachable/shopping/keeping/declining all create one;
-- "renewing" doesn't, so stays null for that outcome), cleared back to null — after deleting that
-- reminder — the moment Undo is pressed.
-- Any row touched-base BEFORE this migration ran has no id here to look up, so Undo on one of
-- those old rows still won't clean anything up — the reminders already sitting in Reminders from
-- before this fix need a one-time manual Delete there; there's no reliable way to auto-match them
-- back to a product after the fact (the message text alone isn't a safe enough link, especially
